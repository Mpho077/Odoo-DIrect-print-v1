...
class DirectPrintAction(models.AbstractModel):
    _name = 'direct.print.action'
    _description = 'Direct Print Action Helper'

    @api.model
    def _get_direct_print_models(self):
    ...
    @api.model
    def _get_direct_print_code_template(self):
        return """
if records:
    action = {
        'name': 'Direct Print',
...
"""
    ...
    def _ensure_print_actions(self):
        ...
        """
        code_template = self._get_direct_print_code_template()
        IrModel = self.env['ir.model']
        IrActionsServer = self.env['ir.actions.server']
        created = []

        for model_name in self._get_direct_print_models():
