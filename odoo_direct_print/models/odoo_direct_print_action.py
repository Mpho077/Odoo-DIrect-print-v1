...
class DirectPrintAction(models.AbstractModel):
    _name = 'odoo.odoo.direct.print.action'
    _description = 'Direct Print Action Helper'

    @api.model
    def _get_odoo_direct_print_models(self):
    ...
    @api.model
    def _get_odoo_direct_print_code_template(self):
        return """
if records:
    action = {
        'name': 'Direct Print',
        'type': 'ir.actions.act_window',
        'res_model': 'odoo.direct.print.wizard',
        'view_mode': 'form',
        'target': 'new',
        'context': {
...
"""
    ...
    def _ensure_print_actions(self):
        ...
        """
        code_template = self._get_odoo_direct_print_code_template()
        IrModel = self.env['ir.model']
        IrActionsServer = self.env['ir.actions.server']
        created = []

        for model_name in self._get_odoo_direct_print_models():
